...
(function () {
  ...
  const selector = 'button:not([disabled]):not(.dialog-close):not(.drawer-close):not(.search-close):not(.remove-item):not(.gallery-arrow):not(.gallery-dot), a[href]:not(.wa-float), label.pay-method, .product-image[role="button"]';
